...
def bout_detect(raw_motion_data):
    ...
    a bout."""
    window_length = 50 # Most bouts last much lesser than 20 frames, so this is a really safe window to search for bouts.
    ...
    for i in nonzero_indices:
        if i not in all_bout_indices:
            ...
            try:
                first_zero_after_start = np.flatnonzero(raw_motion_data[bout_start:bout_start+window_length+1] == 0)[0]
            except IndexError:
                try:
                    first_zero_after_start = np.flatnonzero(raw_motion_data[bout_start:len(raw_motion_data)] == 0)[0]
                except IndexError:
                    first_zero_after_start = len(raw_motion_data)
                    do_not_append = True
            if not do_not_append:
                bout = tuple(raw_motion_data[bout_start-1:bout_start+first_zero_after_start+1]) # Adjusted to start and end at 0
...
directory = r'D:\VT Lab Stuff\Project 01 - Characterizing Auts2 Mutants\C. Free Swimming Experiments\Final Experiment\auts2a mutants\6dpf'
...
threshold = 30 # Set any raw motion value below this to 0. Determined from the raw motion plot for 1000 frames.
upper_limit = 1000 # Set any raw motion value above this to 0. Is definitely noise due to light fluctuations.
