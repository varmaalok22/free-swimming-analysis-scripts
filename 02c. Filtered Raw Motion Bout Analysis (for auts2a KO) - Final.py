...
for j in raw_motion_data.iterkeys():
    fish_id = raw_motion_data[j][0][0][:-4]
    motion = raw_motion_data[j][1]
...
if bout_params_plot:
    ...
    if to_plot[3]:
        ...
        plt.tick_params(
            ...
            labelbottom='off') # labels along the bottom edge are off
        plt.ylabel("Bout Duration (ms)")
        plt.title("Bout Duration Distribution (auts2a -/- %s, pooled)" %dpf)        
    ...
    if to_plot[4]:
        ...
        plt.title("Interbout Interval Distribution (auts2a -/- %s, pooled)" %dpf)
